...
dough::World world = dough::World(((float)1/60), -9.81, 100, 4, 50);
...
int main() {
    ...
    dough::ParticlesGravity* gravityFG = new dough::ParticlesGravity(&world.particles);
    auto i = world.particles.begin();
    for (; i != world.particles.end(); i++) {
        world.getForceRegistry().add(i.base(), gravityFG);
    }
